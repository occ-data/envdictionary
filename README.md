...
The Environmental Data Commons’ (EDC) data dictionary provides the first level of validation for all data
stored in and generated by the EDC. Written in YAML, JSON schemas define all the individual entities
(nodes) in the EDC data model. Moreover, these schemas define all of the relationships (links)
between the nodes. Finally, the schemas define the valid key-value pairs that can be used to
describe the nodes. 

## EDC Data Dictionary Structure 

The EDC Data Model covers all of the nodes within the EDC as well as the relationships between
...
the data model as well as providing a clear and concise representation of the data in the EDC.

Beyond node type, there are also a number of EDC extensions used to further define the nodes within
...
directed graph of the EDC Data Model.
...
`preferred` are still sought by EDC, but submissions without them are allowed. 
